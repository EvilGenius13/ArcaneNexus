...
function createWindow() {
  mainWindow = new BrowserWindow({
    ...
    webPreferences: {
      ...
      enableRemoteModule: true, // Enable remote module for react
    },
  });
...
ipcMain.handle("fetch-manifest", async () => {
  try {
    ...
    if (!storedManifest) {
      ...
      updatesAvailable =
        JSON.stringify(newManifest) !== JSON.stringify(storedManifest);
    }
...
function cleanUpFiles(destination, manifestFiles) {
  ...
  const manifestFilePaths = manifestFiles.map((file) =>
    path.join(destination, file.path)
  );

  const filesToDelete = allFiles.filter(
    (file) => !manifestFilePaths.includes(file)
  );
