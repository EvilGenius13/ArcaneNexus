...
export const getGamesList = async (): Promise<any[]> => {
  ...
  const games = await db.all(
    "SELECT DISTINCT name, description, logo FROM games"
  );
  return games;
};
...
export const getLatestGameVersion = async (name: string): Promise<any> => {
  ...
  const version = await db.get(
    "SELECT version FROM games WHERE name = ? ORDER BY version DESC LIMIT 1",
    name
  );
  return version;
};
...
export const addGame = async (
  ...
  const db: MyDatabase = await initializeDatabase();
  await db.run(
    "INSERT INTO games (name, version, description, logo, jsonBLOB) VALUES (?, ?, ?, ?, ?)",
    name,
    version,
    description,
    logo,
